...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^derp_") ; then
        ...
        export BUILD_NUMBER=$( (date +%s%N ; echo $derp_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        DERP_BUILD=
    fi
    export DERP_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Derp-CAF Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
